...
export class OffsetEdit {
	...
	public static from(contentChanges: readonly vscode.TextDocumentContentChangeEvent[]) {
	...
	constructor(
		...
		for (const edit of edits) {
			if (!(edit.replaceRange.start >= lastEndEx)) {
				throw new Error(`Edits must be disjoint and sorted. Found ${edit} after ${lastEndEx}`);
			}
